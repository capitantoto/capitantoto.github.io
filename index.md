---
layout: default
---

# Gonzalo Barrera Borla

Math nerd. Teacher by heart, engineer by trade. Buenos Aires (remote-friendly).

Statistician / ML engineer. My models have priced programmatic ad auctions
(Jampp), handicapped horse races, flagged crypto-wallet fraud (Muun), and
matched people looking for love (Sitch). The common thread: putting honest
probabilities on messy human behavior, then writing production code around
them.

MSc in Mathematical Statistics (FCEN-UBA) · BA in Economics (FCE-UBA) ·
statistics TA at the Instituto de Cálculo, because explaining something is
the only way I trust that I understand it.

**Open to remote DS / MLE / quant roles.**

## Work & talks

- *Fermat distances in kernel density classifiers* — MSc thesis (2026, dir.
  Pablo Groisman): density-based geodesic distances for nonparametric
  classification, plus a scikit-learn-compatible open-source library.
  [Code & thesis (PDF)](https://github.com/capitantoto/fermat) ·
  [seminar slides](https://github.com/capitantoto/fermat/blob/HEAD/docs/seminario-modesto.pdf) —
  Stochastic Models Seminar, FCEN-UBA
- *Matchmaking and decision-making in data science* — Universidad de San
  Andrés, MCD (2026).
  [Notebook & slides](https://github.com/capitantoto/matchmaking-udesa)
- *Optimización de ofertas: un enfoque dual* — programmatic bid optimization
  at Jampp. [PyData Córdoba 2019](https://pydata.org/cordoba2019/speaker/profile/43/gonzalo-barrera-borla/)
- [pydatajson](https://github.com/datosgobar/pydatajson) — open-data metadata
  tooling for Argentina's national data portal
- Election-night forecasting from partial counts — BA thesis in Economics.
  [Repo](https://github.com/capitantoto/tesis_mesis)

## Writing

Coming soon: the thesis, in plain words, in two posts.

## Elsewhere

[GitHub](https://github.com/capitantoto) ·
[LinkedIn](https://www.linkedin.com/in/gonzalo-barrera-borla-4a6b3711/) ·
[borlandux@outlook.com](mailto:borlandux@outlook.com)

---

<small>Todos a bordo del tototren! <em>— this site, 2014–2026</em></small>
